...
Channel 
    ...
    .splitCsv(skip:1)
    .map{sample, vcf_path,cnv_path -> [sample, file(vcf_path), file(cnv_path), file(header), file(vcftobedpe)]}
    .set{ ch_input }
...
process  CloudOS_MTR_input{
    ...
    script:
    Rscript CloudOS_MTR_input.R  $sample $vcf_path $cnv_path $header $vcftobedpe
}