...
pdf(file=paste0(sample, '/', sample, '_vaf_expected_vaf.pdf'), height=7.5, width=7.8)
plotSample(vcf,bb)
dev.off()

mt$T <- mt$T[-c(1)]

write.csv(mt$T, paste0(sample, '/',sample,'_mT.csv'))
write.csv(mt$V, paste0(sample, '/',sample,'_mV.csv'))
write.csv(mt$V$CLS, paste0(sample, '/',sample,'_CLS.csv'))
