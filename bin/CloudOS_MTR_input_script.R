...
hist(as.numeric(unlist(snvtab['VAF'])), breaks=bins, cex.main=0.5,main = paste0(sampleID, ' (tumour purity = ', tp, ')'), xlab='VAF', col='#fadadd')
dev.off()

pdf(file = paste0(sampleID, '_vaf_hist_normal_cn.pdf'))
hist(as.numeric(unlist(snvtab_normal['VAF'])), breaks=bins, cex.main=0.5, main = paste0(sampleID, ' (tumour purity = ', tp, '), ',  '\n mutations in diploid regions = ', nrow(snvtab_normal), '/', nrow(snvtab)), 
     xlab='VAF', col='#fadadd')
dev.off()
